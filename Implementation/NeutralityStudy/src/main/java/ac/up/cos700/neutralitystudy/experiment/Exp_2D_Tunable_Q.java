...
/**
 * This experiment only differs from 1D_Tunable in the way plots are drawn.
 *
...
public class Exp_2D_Tunable_Q extends Exp_1D_Tunable_Q {
    ...
    protected void finalise() throws Exception {
        ...
        for (int i = 0; i <= numQs; i += (numQs/2)) {
            ...
            Results.newGraph(path, quantisedProblem.getName() + " " + "q" + " = " + decFormat.format(currentQ), "x1", "x2", "f(x)", 3);
            Results.addPlot(null, quantisedProblem);
            Results.plot();

            //graph of quantised problem - showing sample
            Results.newGraph(path, quantisedProblem.getName() + " " + "q" + " = " + decFormat.format(currentQ) + " Sampled", "x1", "x2", "f(x)", 3);
            Results.addPlot(null, quantisedProblem);
