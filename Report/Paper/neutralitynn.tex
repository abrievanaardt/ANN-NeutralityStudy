...
\author{\IEEEauthorblockN{Willem Abraham van Aardt}
...
To the best of the authors' knowledge, there is currently no offline method for quantifying neutrality in continuous search spaces. This paper proposes two normalised measures of neutrality based on a progressive random walk sample. The measures are evaluated on a set of two-dimensional benchmark problems by performing a visual analysis. The relative values acquired on problems of increasing dimensions are then compared to ascertain whether the measures scale well. These measures are then applied to NN error landscapes produced by benchmark classification data sets. The study was restricted to static, continuous, boundary-constrained optimisation problems, and fully-connected feed-forward NNs.

Section \ref{backgroundNeutralityFL} discusses neutrality in more detail. The current picture of the NN error landscape is discussed in Section \ref{backgroundNNEL}. Section \ref{backgroundDiscreteNeutrality} provides background on neutrality measures used in discrete spaces. Measures proposed in this paper are discussed in Section \ref{continuousNeutrality}, and analysed in Sections \ref{visualAnalysis} and \ref{nnELApplication}.
...
In order to characterise neutrality it is necessary to provide a definition for neighbourhood. In discrete spaces, neighbouring solutions of any solution $\vec{x}_j$ are defined as those solutions that can be reached by applying an operator to $\vec{x}_j$. Determining such neighbours for real-encoded vectors would be an infinite process. Instead, a neighbourhood is defined as all solutions within the hypercube with $\vec{x}_j$ as centre point and length of side $s$. The sampling method used in this study (see section \ref{sampling}) takes steps along the axes of the hypercube, with step sizes up to \textit{stepSize}, where \textit{stepSize} = $\frac{1}{2}\times s$. Furthermore, a three-point structure is wholly regarded as neutral when, for neighbouring solutions $\vec{x}_{j-1}$, $\vec{x}_j$ and $\vec{x}_{j+1}$, the following condition holds:

\begin{equation}
\label{eqNeutralityDef}
	f(\vec{x}_{i^{\text{max}}}) - f(\vec{x}_{i^{\text{min}}}) \le \epsilon			 
\end{equation}
where $i^{\text{max}} = \text{argmax}_i\{f(\vec{x}_i)\}$, $i^{\text{min}} = \text{argmin}_i\{f(\vec{x}_i)\}$, $i \in \{j-1, j, j+1\}$, and $f(\cdot)$ refers to the quality (fitness) of the solution. The next section describes the sampling algorithm used to generate the three-point objects.
...
Two metrics are extracted from the neighbourhood information provided by a progressive random walk, $\mathcal{W}$:
\begin{itemize}
...
where $s_{\text{neutral}}$ is the number of neutral 3-point structures in $\mathcal{W}$ (see Equation~(\ref{eqNeutralityDef})), and $\lvert \mathcal{W} \rvert$ is the total number of 3-point structures in $\mathcal{W}$.
	\item Measure 2 (${\mathcal{M}_2}$) - The length of the largest subsequence of neutral structures in $\mathcal{W}$, proportionate to $\lvert \mathcal{W} \rvert$. This measure estimates the relative size of the largest neutral region. ${\mathcal{M}_2}$ is defined as:
          \begin{equation}
            \label{eqNeutralityM2}
	    {\mathcal{M}_2} = \frac{\lvert w_{\text{max}} \rvert}{\lvert \mathcal{W} \rvert}
          \end{equation}
where $w_{\text{max}}$ is the longest sub-walk of $\mathcal{W}$ consisting of neutral 3-point objects only.
\end{itemize}
...
The generic algorithm for applying measures ${\mathcal{M}_1}$ and ${\mathcal{M}_2}$ to samples generated using progressive random walks is summarised in Algorithm \ref{algNeutralityMeasure}. Note that the sampled fitness values are normalised to the $[0,1]$ interval. This is done to ensure that the same $\epsilon$ value used in Equation~(\ref{eqNeutralityDef}) can be used across problems of varied fitness scale.
...
\begin{algorithm}[!t]
	...
	\begin{algorithmic}
		...
		\FOR{$\forall i \in \{1,...,n_w\}$}
			\STATE \textit{walk} $\gets$ sample the input problem using a progressive random walk~\cite{malan2014progressive};
			\STATE Normalise the sample fitness range in \textit{walk} to $[0,1]$;
			\IF{$\mathcal{M}$ is ${\mathcal{M}_1}$}
				\STATE $n_s \gets$ determine the number of neutral structures in \textit{walk};
			\ELSE
				\STATE $n_s \gets$ determine the length of the longest neutral subsequence in \textit{walk};
			\ENDIF
...
Seven functions, summarised in Table~\ref{functionBenchmarks} and Figure~\ref{figVisBenchmark}, were selected for the analysis. These benchmarks exhibit a varying degree of neutrality, from the completely neutral landscape of $f_{\text{Fl}}$  to the landscape of $f_{\text{AV}}$, in which no neighbouring solutions have equal fitness. For reference, the functions were ranked from most to least neutral, as identified by the authors. When the proposed measures are applied to the benchmarks, it should be expected that they rank the functions in the same order. Note that $f_{\text{Ea}}$ is the only problem defined in two dimensions only, and will therefore not be used for scalability testing. Also note that $f_{\textit{St}_2}$ is the same as $f_{\textit{St}_1}$, except zoomed in on a smaller domain. The small plateaus that occur on multiple levels of the landscape are clear from the plot of $f_{\textit{St}_2}$. From the perspective of FLA, although these functions have the same definitions, the difference in their domain implies they will not be treated the same by either an optimisation algorithm or any FLA technique. $f_{\text{Ea}}$ is less neutral than $f_{\text{IT}}$ due to the soft edges and sharp point of the protruding cone. Although the rest of the surface may look more neutral than the surface of $f_{\text{IT}}$, the impact of the gradients near the cone cannot be appreciated at this scale.
...
\begin{itemize}
	\item Algorithm \ref{algNeutralityMeasure} was applied 30 independent times for each neutrality measure, to each of the benchmark problems for $n=2$, to account for the stochastic nature of the sampling algorithm. From the 30 measurements, the averages and standard deviations were obtained. This was repeated for dimensions 3, 4, 5, 10, and 15.
	\item \citet{malan2014progressive} suggest the ideal number of walks to perform is equal to $2^n$, the number of starting zones, but they also note that this can be infeasible and unnecessary. The number of walks was therefore set one order of magnitude greater than the dimension of the problem, $10n$.
	...
	\item For $\mathcal{M}_2$ to be representative, the walk cannot be significantly longer than the domain of the problem. This would render the length of any longest neutral sequence an inconsequential fraction of the walk length. Thus, $numSteps$ was set to 200 such that the landscape would, on average, be traversed only once.
	\item Neutrality threshold $\epsilon$ was set to 1.000E-8, often used in optimisation competitions as an acceptable trade-off to the known minimum of an objective function.
\end{itemize}


\subsection{Results}
\label{visualResults}
Table \ref{tblStaticProblems2} shows the averages and standard deviations obtained for ${\mathcal{M}_1}$ and ${\mathcal{M}_2}$ for two dimensions on the chosen benchmarks. The results establish that both measures are largely in agreement with the rank that was assigned to the problems. The only exception is $f_{\text{Ea}}$: According to ${\mathcal{M}_2}$, $f_{\text{Ea}}$ should be ranked as more neutral than $f_{\text{IT}}$. Indeed, $f_{\text{Ea}}$ is asymmetric in the given domain, therefore a random walk started away from the single present peak is less likely to encounter a change in fitness. Thus, a random walk is more likely to generate a longer neutral sequence. However, ${\mathcal{M}_1}$ indicates that the overall amount of neutrality is lower in $f_{\text{Ea}}$, correctly ranking it as less neutral than $f_{\text{IT}}$.  
...
The measures appear to be consistent based on the low standard deviation values, which are frequently in the order of $10^{-3}$. As expected, $f_{\textit{St}_2}$ exhibited neutrality a few orders of magnitude greater than its zoomed-out counterpart. $f_{\text{IA}}$ and $f_{\text{IT}}$ have similar values for ${\mathcal{M}_1}$. The two functions both have roughly the same level of neutrality present since both are completely flat apart from the 4 edges that walks would have to cross. The area covered by these edges is larger for $f_{\text{IT}}$, which would imply that a progressive walk would encounter these steep gradients more frequently, resulting in fewer neutral structures. The larger difference in measurements when using ${\mathcal{M}_2}$ on $f_{\text{IT}}$ and $f_{\text{IA}}$ can be explained by realising that the narrower protrusion in $f_{\text{IA}}$ would account for very large surrounding neutral regions, while the protrusion in $f_{\text{IT}}$ divides the space of neutral regions roughly in half, producing smaller neutral regions overall. 

The values for the extreme cases of $f_{\text{Fl}}$ and $f_{\text{AV}}$ confirm that the measures correctly identify problems that are entirely flat and those that have no flat regions. Interestingly, ${\mathcal{M}_2}$, which indicates the relative size of neutral regions in the landscape, differs by 2 orders of magnitude between  $f_{\textit{St}_2}$ and  $f_{\textit{St}_1}$. This corresponds to the fact that $f_{\textit{St}_1}$ is simply $f_{\textit{St}_2}$ zoomed out by 1 order of magnitude in each dimension, thus the size of all the steps in the landscape is increased by the same amount. Similar behaviour is observed for ${\mathcal{M}_1}$
...
Tables \ref{tblStaticProblemsNM1} and \ref{tblStaticProblemsNM2} show the values for measures ${\mathcal{M}_1}$ and ${\mathcal{M}_2}$ on problems whose dimensions were scaled up. It is evident from both tables that the ranking order established in two dimensions is preserved across higher dimensions, as indicated by the colour coding. The measures maintain the same level of consistency as in two dimensions. It is worth noting the tendencies for both measures, on all problems, to gravitate to 0 or 1 as the dimensions increase (see Figure~\ref{figNeutralityVsDimensions}). This behaviour was thought to be associated with the insensitivity of algorithm parameters such as the \textit{stepSize} or the under-representative number of walks, but experiments that were repeated with extreme parameter values, computationally infeasible in practice, showed that, although more accurate, the measures displayed the same tendencies. For reference, the number of walks were set to $2^n$ and \textit{numSteps} for ${\mathcal{M}_1}$ was set to 2000 when the asymptotic behaviour was still apparent. Thus, an increase in dimensionality ultimately changes the perception of the landscape. This phenomenon is also known as the curse of dimensionality. Increase in dimensionality increases the likelihood of encountering a fitness change in at least one of the many dimensions, provided there is enough variability in the landscape. On the other hand, the likelihood of fitness change is diminished if the variability is limited, especially if some dimensions exhibit more variability than others. Scarce variability implies that adding extra dimensions makes it harder to locate the change in the landscape. In both cases the observed behaviour is due to the changed perception of the fitness landscapes in higher dimensions, a phenomenon worthy of an extended investigation. Perhaps the neutrality metrics can be used to evaluate the asymptotic behaviour of optimisation problems, and to estimate the speed of degradation of the information present in the landscape, associated with increased problem dimensionality.

\begin{figure}[!ht]
	\centering
	\subfloat[]{\includegraphics[width=0.9\linewidth]{neutralityVsDimensionsM1}}%
	\label{figNeutralityVsDimensionsM1}	
		
	\subfloat[]{\includegraphics[width=0.9\linewidth]{neutralityVsDimensionsM2}}%
...
\section{Neutrality of Neural Network Error Landscapes}
\label{nnELApplication}
One area of application for the proposed neutrality measures is the study of NN error landscapes. As noted in Section \ref{backgroundNNEL}, it has long been thought that the NN error surface consists of large neutral regions such as plateaus and ridges. This section of the paper applies ${\mathcal{M}_1}$ and ${\mathcal{M}_2}$ to some well-known benchmark data sets for classification, and successfully confirms the theoretical hypothesis.

\subsection{Benchmark Problems}
\label{nnBenchmarks}
For this phase of the study, apart from the XOR problem, benchmarks of varying complexity were obtained from the UCI Machine Learning Repository \cite{Lichman2013}. Optimised NN architectures for each benchmark were employed, summarised in Table~\ref{tblNNBenchmarks}. The study was limited to feed-forward NNs, using the tanh \cite{lecun1989generalization} symmetric activation function ($1.7159\cdot tanh(\frac{2}{3}x)$) and the mean squared error (MSE) loss function. All pattern inputs and outputs in the benchmark suite were scaled to the active domain and range of the tanh function, $[-1,1]$.
...
\begin{itemize}
	\item Algorithm \ref{algNeutralityMeasure} was again applied 30 independent times for each neutrality measure, to each of the benchmark problems in Table~\ref{tblNNBenchmarks}. This was done for various values of $x_{\text{max}}$, with the domain of the sample in all dimensions defined as $[-x_{\text{max}}, x_{\text{max}}]$ and $x_{\text{max}} > 0$.
	\item Sampling the NN problems was prohibitively expensive, therefore the number of walks was restricted to $2n$.
	\item All other parameters remained unchanged.
\end{itemize}


\subsection{Results}
\label{nnResults}
Figure \ref{figNNNeutralityVsDomain} shows the neutrality measured for the various NN problems using ${\mathcal{M}_1}$ and ${\mathcal{M}_2}$, respectively, for increasing domains centred around the origin of the MSE search space. Note the scale applied to the x-axis for readability. The first observation that can be made is the similarity in the behaviour of both measures across all problems: 
\begin{itemize}
  \item Some neutrality is observed around the origin.
  \item No neutrality is observed for $1 < x_{\text{max}} < 10$.
  \item Gradually increasing neutrality is observed for $x_{\text{max}} > 10$.
\end{itemize}
These results correspond well with the existing insight. Flatness about the origin has been noted before as a characteristic of symmetric activation functions \cite{lecun2012efficient}. Iris and glass do not exhibit neutrality around the origin, however -- this can be attributed to the fact that iris and glass are the only two problems with more than 1 output. The relationship between NN architecture and the corresponding neutrality is a topic of future research.

\begin{table}[!ht] 
	\renewcommand{\arraystretch}{1.3}
	\caption{Benchmark data sets and their corresponding optimal three-layer NN architecture, in the format \#inputUnits-\#hiddenUnits-\#outputUnits}
	\label{tblNNBenchmarks}
	\centering
	\begin{tabular}{|l|l|}
		\hline
		\textbf{Data set}		& \textbf{Architecture} \\ \hline				
		XOR									&  2-2-1		\\
...
\begin{figure}[!t]
	...
	\label{figNNM2}	
	\caption{Degree of neutrality measured versus the sample bounds on NN benchmarks in section \ref{nnBenchmarks} using (a) ${\mathcal{M}_1}$ and (b) ${\mathcal{M}_2}$. The domain of each dimension, $x_i$, has been fixed to $[-x_{\text{max}}, x_{\text{max}}]$, with $x_{\text{max}} > 0$}
	\label{figNNNeutralityVsDomain}
\end{figure}

No neutrality for $1 < x_{\text{max}} < 10$ indicates that this part of the search space contains a lot of information to potentially guide an optimisation algorithm. Indeed, non-zero weights of relatively small magnitude can easily change the output of a NN, i.e. the resulting NN fitness. If ridges are present here, they are not too far apart, and a random walk steps over a few on its course, observing frequent changes in fitness.

Finally, increasing neutrality for $x_{\text{max}} > 10$ corresponds to growing plateaus associated with neuron saturation. Regions as far as $x_{\text{max}} = 1000$ correspond to large absolute NN weight values that saturate the activation functions, causing all nodes to produce outputs close to the asymptotic values of the activation functions. A saturated neuron becomes insensitive to small fluctuations in the weights, causing the NN fitness to remain the same even if some of the weights change their value. The neutrality induced by the saturation phenomenon is evident from the results.

Interestingly, iris and glass, even though they follow the general trend of increasing saturation, exhibit lower levels of neutrality than the remaining problems. It has already been pointed out that iris and glass have more than one output neuron. Thus, the fitness of the resulting NN is the composite fitness of the numerous outputs. If the fitness of at least one of the few outputs changes, the overall fitness of the NN would change, too.

\section{Conclusion}
This paper proposed two complementary measures of neutrality, ${\mathcal{M}_1}$ and ${\mathcal{M}_2}$, based on a progressive random walk sample of the landscape, to characterise a search landscape in terms of the presence and extent of neutral regions. The measures were tested on a relatively small set of benchmark functions that can be expanded in future research. The definition of the neutrality measures also allows for various levels of sensitivity to be adjusted, optimal values of which have yet to be determined empirically in an extensive study. Neutrality threshold $\epsilon$ can be increased in cases where the detection of near-neutrality is important. 
...
The proposed measures could be used to study the relationship between the degree of neutrality and search algorithm performance, as well as the degree of neutrality and the corresponding NN problem attributes, and could ultimately be added to the FLA practitioner's arsenal to perform algorithm selection based on problem characteristics.
